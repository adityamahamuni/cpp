...
void find_test() {
	...
	std::cout << "\n========================" << std::endl;
	std::list<Person> players{ {"Adi", 21}, {"Sukhada", 22} };

	auto loc1 = std::find(players.begin(), players.end(), Person{ "Adi", 21 });

	if (loc1 != players.end())
		std::cout << "Found Adi" << std::endl;
	else
		std::cout << "Adi not found!" << std::endl;
}
...
void count_if_test() {
	...
	std::cout << "\n========================" << std::endl;
	num = std::count_if(vec.begin(), vec.end(), [](int x) {return x > 3; });
	std::cout << num << " of elements are greater than 3" << std::endl;
}
